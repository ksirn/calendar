...
async function hasAcceptedConnection(user1Id, user2Id) {
    if (user1Id === user2Id)
        return true;
...
router.get('/', async (req, res) => {
    try {
        ...
        const outgoing = await prisma_1.default.todo.findMany({
            where: {
                creatorId: userId,
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        const body = (req.body ?? {});
        const todo = await prisma_1.default.todo.findUnique({ where: { id: todoId } });
        if (!todo)
        ...
        if (body.eventId !== undefined)
            updates.eventId = body.eventId ? String(body.eventId) : null;
        const updated = await prisma_1.default.todo.update({ where: { id: todoId }, data: updates });
...
router.post('/:id/schedule', async (req, res) => {
    try {
        ...
        // Проверка конфликта
        const existing = await prisma_1.default.event.findMany({
            where: { ownerUserId: userId, status: 'active' },
        });
        const hasConflict = existing.some((e) => startAt < e.endAt && endAt > e.startAt);
        if (hasConflict) {
        ...
        const updated = await prisma_1.default.todo.update({
            where: { id: todoId },
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await prisma_1.default.todo.update({ where: { id: todoId }, data: { status: 'deleted' } });
...
router.post('/:id/cancel', async (req, res) => {
    try {
        ...
        await prisma_1.default.todo.update({ where: { id: todoId }, data: { status: 'cancelled' } });
        return res.json({ ok: true });
    }
    catch (error) {
