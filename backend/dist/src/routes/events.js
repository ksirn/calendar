...
async function validateParticipants(creatorId, participants) {
    const uniqueParticipants = Array.from(new Set(participants.filter((participantId) => participantId !== creatorId)));
...
async function createSingleEvent(input) {
    const { creatorId, ownerUserId, title, emoji, description, startAt, endAt, blockType, participants, } = input;
...
router.get('/', async (req, res) => {
    try {
        ...
        const connections = otherUserIds.length
            ? await prisma_1.default.connection.findMany({
                where: {
        ...
        for (const connection of connections) {
            const otherUserId = connection.userAId === viewerUserId ? connection.userBId : connection.userAId;
        ...
        const maskedEvents = events.map((event) => {
            ...
            const visibility = connection.userAId === event.ownerUserId
